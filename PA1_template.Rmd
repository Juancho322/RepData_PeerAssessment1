...
hist(hist.data$totalsteps, breaks = seq (0, 25000, 1000), col = "red", 
...
```
The mean total number of steps taken per day is `r mean.total` steps, and the median total number of steps taken per day is `r median.total` steps.
...
plot(interval, mean5min.list, type = "l", 
     main = "The average number of steps taken per 5 mintues interval", 
     xlab = "Interval", ylab = "Number of steps")
```
```{r}
max.5min <- interval[which(mean5min.list == max(mean5min.list))]
```
The interval at `r max.5min` mintues on average across all the days in the dataset, contains the maximum number of steps.
...
```
The total number of missing values in the dataset is `r miss.cases`.
...
hist(hist.data1$totalsteps, breaks = seq (0, 25000, 1000), col = "red", 
     main = "Histogram of the total number of steps taken each day", 
     xlab = "Number of steps")
...
```
After filling NAs, the mean total number of steps taken per day `r flagmean` different, and the median total number of steps taken per day `r flagmedian` different.
...
interval.df <- data.frame(steps = c(mean5min.weekday, mean5min.weekend), 
...
```
There `r flagdif` difference in activity patterns between weekdays and weekends. Weekdays are `r flaglevel` active than weekends.
